...
//=================CONTROL STEP MOTOR (checked tempt)
void ForwardDC(uint16_t l_pulseIn, uint16_t timeDelay)
{
	...
	for(uint16_t i =0; i < l_pulseIn; i++)
	{
		...
		{
			flag_send = false;
			if(flag_getEncoderStep) EncodeDataDC(Data_Decode);
		}
	}
}
...
void Control_Motor(float DELTA)
{
	if(Mode == AC_SERVO)
	{
		float deltaPulse = DELTA*GearboxAC_DtoP; //number 15*1000/360
	...
	if(Mode == STP_MOTOR)
	{
		float deltaPulse = DELTA*GearboxStep_DtoP; //number 3264*13.7/360
		pulseEnd += deltaPulse - (int32_t)deltaPulse;
		int32_t pulseSupply = deltaPulse + (int32_t)pulseEnd/1;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		if(RxDataUart[9] == getCheckSum(RxDataUart, RxBufferSize-1) && RxDataUart[0] == 251)
		{
			Check_Data = 1;
			RxSaveUart[0] = RxDataUart[0];
			RxSaveUart[1] = RxDataUart[1];
			RxSaveUart[2] = RxDataUart[2];
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_GPIO_WritePin(GPIOB, STP_Pin, GPIO_PIN_SET);
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
		...
		if(flag_send == true && flag_enable_send == true)
		{
			...
			TX_CAN_HEADER.StdId = 0x010; //Send to Master
